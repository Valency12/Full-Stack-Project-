...
function handleLogin(e) {
    ...
    // Intentar validar con la base de datos si está disponible
    ...
    // Modo demo: Login sin base de datos (para diseño)
    // Permite login con cualquier email/password para probar el diseño
...
function handleRegister(e) {
    ...
    // Verificar si el usuario ya existe (si hay base de datos)
    if (typeof Database !== 'undefined' && Database) {
        const existingUser = Database.getUserByEmail(email);
        if (existingUser) {
            showAuthError('registerForm', 'Este correo electrónico ya está registrado. Por favor, inicia sesión.');
            return;
        }
        ...
        if (newUser) {
            // Configurar el usuario actual inmediatamente
            state.currentUser = {
                id: newUser.id,
                email: newUser.email,
                name: newUser.name,
                type: newUser.user_type,
                imageUrl: newUser.image_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=400&h=400&fit=crop',
                description: newUser.description || '',
                techStack: newUser.tech_stack || []
            };
            ...
            showAuthError('registerForm', 'Error al crear la cuenta. Por favor, intenta nuevamente.');
            return;
        }
    }
